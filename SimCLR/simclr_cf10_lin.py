...
#     version_base="1.1",
#     config_path=".",
#     config_name="simclr_config_18.yml"
#     )
...
#     pre_model.load_state_dict(torch.load('/mnt/disk4/baodq/SimCLR-CIFAR10/logs/SimCLR/cifar10/simclr_resnet18_epoch200.pt'))
...
def finetune(args:  DictConfig) -> None:
    ...
    # FIX 1: Use more samples for few-shot learning (e.g., 100 per class = 1000 total)
    # Or use all training data for full linear evaluation
    samples_per_class = getattr(args, 'samples_per_class', 100)  # Default 100 per class
    n_samples = samples_per_class * n_classes
    indices = np.random.choice(len(train_set), n_samples, replace=False)
    sampler = SubsetRandomSampler(indices)
    
    # FIX 2: Use smaller batch size for few-shot, or remove drop_last
    # For linear eval, batch_size should be <= n_samples
    finetune_batch_size = min(args.batch_size, n_samples)
    
    train_loader = DataLoader(
        train_set, 
        batch_size=finetune_batch_size, 
        drop_last=False,  # FIX 3: Don't drop last batch with small datasets
        sampler=sampler
    )
    test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=False)
    
    # Verify we have batches
    logger.info(f"Training samples: {n_samples}, Batch size:  {finetune_batch_size}, "
                f"Batches per epoch: {len(train_loader)}")
    
    if len(train_loader) == 0:
        raise ValueError(f"No training batches!  n_samples={n_samples}, batch_size={finetune_batch_size}")

    # Prepare model
    base_encoder = eval(args.backbone)
    pre_model = SimCLR(base_encoder, projection_dim=args.projection_dim).cuda()
    pre_model.load_state_dict(torch.load(
        '/mnt/disk4/baodq/SimCLR-CIFAR10/logs/SimCLR/cifar10/simclr_resnet18_epoch200.pt'
    ))
    
    model = LinModel(pre_model.enc, feature_dim=pre_model.feature_dim, n_classes=n_classes)
    model = model.cuda()

    # FIX 4: Properly freeze encoder parameters
    for param in model.enc.parameters():
        param.requires_grad = False
    
    # Only linear layer parameters are trainable
    parameters = list(model.lin.parameters())
    logger.info(f"Trainable parameters: {sum(p.numel() for p in parameters)}")

    optimizer = torch.optim.SGD(
        parameters,
        lr=0.2,   # lr = 0.1 * batch_size / 256, see section B.6 and B.7 of SimCLR paper.
        momentum=args.momentum,
        weight_decay=0.,
        nesterov=True
    )
    ...
    scheduler = LambdaLR(
        ...
        lr_lambda=lambda step: get_lr(
            step,
            total_steps,
            1.0,  # lr_lambda computes multiplicative factor (starts at 1.0)
            1e-3 / 0.2  # min_lr / base_lr as ratio
        )
    )
